<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=22&duration=2500&pause=800&color=87A96B&center=true&vCenter=true&width=500&lines=hi%2C+I%27m+Priscilla+%F0%9F%91%8B;swe+%40+nomura+%C2%B7+london;currently+building+xlstream)](https://github.com/cilladev)

</div>

---

### about

- 🔭 software engineer at [Nomura](https://www.nomura.com/) — Global Markets Tech, London
- 🍵 outside the screen: running, lifting, bouldering, suspiciously serious about matcha

`Python` &nbsp; `TypeScript` &nbsp; `FastAPI` &nbsp; `Next.js` 

---

### open source contributions


<details open>
<summary><b>2026</b></summary>

<br>

| PR | What it did |
|:--|:--|
| [`tafia/calamine#645`](https://github.com/tafia/calamine/pull/645) | Fixed LOG10 → LOG11 corruption in shared formulas |
| [`PSU3D0/formualizer#81`](https://github.com/PSU3D0/formualizer/pull/81) | Fixed =-2^2 returning -4 instead of 4 (unary minus precedence vs Excel) |

</details>


---

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=cilladev&show_icons=true&hide_border=true" height="140" alt="github stats" />
</div>





